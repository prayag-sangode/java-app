pipeline {
    ...
    stages {
        ...
        stage ("Helm App Deployment")
            ...
            steps {
             withCredentials([file(credentialsId: 'gcp-auth-id', variable: 'GC_KEY')]) {
                sh("sed -i 's|asia-east1-docker.pkg.dev/mypoc-374706/apps-repo/java-app:.*|asia-east1-docker.pkg.dev/mypoc-374706/apps-repo/java-app:${env.BUILD_NUMBER}}|' ./chart/templates/deployment.yaml ")
                sh("gcloud container clusters get-credentials gke-cluster --zone asia-east1-b --project mypoc-374706")
                sh("helm upgrade java-app ./charts --install --values=./chart/values.yaml")
                }
             }
        }
    }    
